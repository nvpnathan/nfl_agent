...
def test_override_pick():
    resp = client.post("/week/2024/10/override", json={
        "game_id": "g1", "confidence_points": 10, "reason": "gut feel"
    })
    assert resp.status_code == 200
    assert resp.json()["confidence_points"] == 10

    # Verify persisted
    resp2 = client.get("/week/2024/10")
    assert resp2.json()["assignments"][0]["confidence_points"] == 10
    assert resp2.json()["assignments"][0]["is_overridden"] == 1
